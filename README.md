...
node site/test-release-manifest.mjs
node scripts/build-site-release-manifest.mjs --check
```

The site binds its version, updated-at provenance, and download control to one verified GitHub Release through `site/release-manifest.json`, generated by `scripts/build-site-release-manifest.mjs --tag <release-tag>` from the release record and its SHA-256 lines. The site is deployed by `.github/workflows/pages.yml` on pushes to `main` that touch `site/`.
