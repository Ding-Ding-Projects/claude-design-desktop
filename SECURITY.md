...
- Stable identity fields are package `@ding-ding-projects/claude-design-desktop`, app ID `com.dingdingprojects.claudedesigndesktop`, executable `Claude Design Desktop.exe`, protocol `claude-design-desktop://`, and local data root `%LOCALAPPDATA%\Ding-Ding-Projects\ClaudeDesignDesktop`. The public URL is `https://ding-ding-projects.github.io/claude-design-desktop/`.
...
The `site/` preview uses local scripts and styles, a strict Content Security Policy, versioned browser storage, and a bounded worker for regular-expression matching. It must not transmit visitor settings, uploaded local images, lock metadata, article searches, or other visitor-owned state. Source checks validate these declarations, but deployed-response headers, browser storage behavior, accessibility, and network silence still require real hosted verification before stable release.
