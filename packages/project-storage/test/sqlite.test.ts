...
async function run(): Promise<void> {
  ...
  try {
    ...
    for (const required of ["account_slots", "projects", "project_grants", "project_files", "design_systems", "chats", "chat_messages", "account_thread_bindings", "comments", "comment_replies", "previews", "settings", "background_operations", "migration_receipts", "history_revisions"]) assert.ok(tableNames.includes(required), `missing SQLite table ${required}`);
    sqlite.close();
  } finally {
    database.close();
