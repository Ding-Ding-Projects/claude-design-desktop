...
export interface AccountHostOptions { accountsRoot: string; appVersion: string; resourcesRoot?: string; codexExecutable?: string; openExternal?: (url: string) => Promise<void> | void; spawn?: SpawnFn; requestTimeoutMs?: number; environment?: NodeJS.ProcessEnv; prepareRemoval?: (slotId: string) => Promise<AccountRemovalImpact>; }
interface ActiveOperation { threadId: string; turnId: string; interruptRequested: boolean; }
interface ActiveSession { slotId: string; generation: number; client: AppServerClient; operations: Map<string, ActiveOperation>; }
...
export class AccountHost implements AccountHostApi {
  ...
  async cancelLogin(loginId: string): Promise<void> { const current = this.login; if (!current || current.loginId !== sanitizeLoginId(loginId)) return; await this.finishLogin(current.loginId, false); }
  ...
  async startTurn(input: TurnInput): Promise<TurnSnapshot> { assertId(input.threadId, "thread id"); if (!Array.isArray(input.input) || input.input.length > 100) throw new Error("Invalid turn input"); const active = this.requireActiveSession(); if (active.operations.size > 0) throw new Error("An active turn must complete or be interrupted first"); const turn = toTurnSnapshot(await active.client.request("turn/start", input as unknown as Record<string, unknown>)); if (!turn) throw new Error("App-server returned an invalid turn"); active.operations.set(turn.id, { threadId: input.threadId, turnId: turn.id, interruptRequested: false }); return turn; }
  async interruptTurn(threadId: string, turnId: string): Promise<void> { assertId(threadId, "thread id"); assertId(turnId, "turn id"); const active = this.requireActiveSession(); const operation = active.operations.get(turnId); if (!operation || operation.threadId !== threadId) throw new Error("Unknown active turn"); operation.interruptRequested = true; await active.client.request("turn/interrupt", { threadId, turnId }); }
  ...
  private async handleNotification(slotId: string, client: AppServerClient, generation: number, method: string, params: Record<string, unknown>): Promise<void> {
    ...
    if (method === "turn/started") { const turnRecord = asRecord(params.turn); const turnId = typeof turnRecord.id === "string" ? turnRecord.id : typeof params.turnId === "string" ? params.turnId : ""; if (turnId && !this.active.operations.has(turnId)) this.active.operations.set(turnId, { threadId: typeof params.threadId === "string" ? params.threadId : "", turnId, interruptRequested: false }); }
    if (method === "turn/completed" || method === "turn/aborted") { const turnRecord = asRecord(params.turn); const turnId = typeof turnRecord.id === "string" ? turnRecord.id : typeof params.turnId === "string" ? params.turnId : ""; this.active.operations.delete(turnId); }
    const event = toOperationEvent(method, params); if (event) this.emit(event);
  }
  private async maybeCompleteLogin(): Promise<void> { const current = this.login; if (!current || current.completing || !current.completed || current.success === null || (current.success ? !current.accountUpdated : false)) return; current.completing = true; await this.finishLogin(current.loginId, current.success); this.emit({ type: "loginCompleted", slot: this.store.get(current.slotId), loginId: current.loginId, success: current.success }); }
...
function toThreadSummary(value: unknown): ThreadSnapshot["thread"] | undefined { const record = asRecord(value); if (typeof record.id !== "string" || record.id.length > 256) return undefined; return { id: record.id, sessionId: typeof record.sessionId === "string" ? record.sessionId : null, name: typeof record.name === "string" ? record.name.slice(0, 512) : null, status: typeof record.status === "string" ? record.status.slice(0, 80) : null, cwd: typeof record.cwd === "string" ? record.cwd.slice(0, 4096) : null }; }
function toTurnSnapshot(value: unknown): TurnSnapshot | undefined { const record = asRecord(value); const turn = asRecord(record.turn ?? value); if (typeof turn.id !== "string" || turn.id.length > 256) return undefined; return { id: turn.id, status: typeof turn.status === "string" ? turn.status.slice(0, 80) : null, errorMessage: typeof asRecord(turn.error).message === "string" ? asRecord(turn.error).message.slice(0, 1000) : null }; }
