...
describe("account host", () => {
  ...
  it("maps local image handles through the authorized resolver instead of exposing paths", async () => {
    ...
    const challenge = await host.startLogin({ flow: "deviceCode" }); await host.cancelLogin(challenge.loginId); const slot = (await host.list())[0]!; await host.activate(slot.slotId); await assert.rejects(() => host.startTurn({ threadId: "thr_1", input: [{ type: "image", url: "https://example.com/image.png" }] }), /policy/); const turn = await host.startTurn({ threadId: "thr_1", input: [{ type: "localImage", projectFileHandle: "file-handle-1" }] }); assert.equal(seenHandle, "file-handle-1"); fake.children[1]!.notify("turn/completed", { threadId: "thr_1", turn: { id: turn.id, status: "completed" } }); await new Promise((resolve) => setImmediate(resolve)); await host.close();
  });
