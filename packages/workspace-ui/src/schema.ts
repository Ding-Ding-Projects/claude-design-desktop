...
function nullableString(value: unknown, path: string, maxLength: number): string | null | undefined {
...
function isoDate(value: unknown, path: string): string | undefined {
  ...
  if (result !== undefined && !Number.isFinite(Date.parse(result))) throw new BridgeSchemaError(path, "ISO date");
  return result;
}
...
const shareRoles = new Set<ShareRole>(["can-edit", "can-comment", "can-view"]);

export function parseAccountSlot(value: unknown, path = "account"): AccountSlot {
  const item = record(value, path);
  assertKnownFields(item, new Set(["slotId", "label", "email", "plan", "lastVerified", "bundledVersion", "state", "expiresAt", "rateLimits"]), path);
  ...
  return { slotId: idValue(item.slotId, `${path}.slotId`), label: boundedString(item.label, `${path}.label`, 200), email: nullableString(item.email, `${path}.email`, 320), plan: nullableString(item.plan, `${path}.plan`, 100), lastVerified: nullableString(item.lastVerified, `${path}.lastVerified`, 64), bundledVersion: nullableString(item.bundledVersion, `${path}.bundledVersion`, 100), state, expiresAt: isoDate(item.expiresAt, `${path}.expiresAt`), rateLimits };
}
...
export function parseDesignSystem(value: unknown, path = "designSystem"): DesignSystem {
  const item = record(value, path);
  assertKnownFields(item, new Set(["id", "name", "version", "tokenCount", "updatedAt"]), path);
...
export function parseFile(value: unknown, path = "file"): WorkspaceFile {
  const item = record(value, path);
  assertKnownFields(item, new Set(["path", "kind", "language", "size"]), path);
...
export function parseLifecycleEvent(value: unknown): AccountLifecycleEvent {
  ...
  assertKnownFields(item, new Set(["type", "slot", "slotId", "rateLimits"]), "event");
  ...
  if (type === "rate-limit-updated") {
    ...
    assertKnownFields(limits, new Set(["remaining", "resetAt"]), "event.rateLimits");
...
export function createDesignerBridge(host: DesignerHost): DesignerBridge {
  ...
  return {
    ...
    async streamChat(projectId, prompt, operationId, onEvent, signal) { const id = idValue(operationId, "operationId"); let terminal = false; let finishTerminal: (() => void) | undefined; const terminalReached = new Promise<void>((resolve) => { finishTerminal = resolve; }); const unsubscribe = host.subscribeChat(id, (raw) => { const event = record(raw, "chat.event"); const eventId = idValue(event.operationId, "chat.event.operationId"); if (eventId !== id || terminal) return; const type = stringValue(event.type, "chat.event.type"); if (type !== "chunk" && type !== "complete" && type !== "cancelled" && type !== "error") return; const parsed: ChatStreamEvent = { operationId: eventId, type, chunk: optionalString(event.chunk, "chat.event.chunk"), message: optionalString(event.message, "chat.event.message") }; onEvent(parsed); if (type === "complete" || type === "cancelled" || type === "error") { terminal = true; finishTerminal?.(); } }); const timer = setTimeout(() => { if (terminal) return; terminal = true; onEvent({ operationId: id, type: "error", message: "Chat stream timed out before a terminal event." }); finishTerminal?.(); }, CHAT_STREAM_TIMEOUT_MS); const onAbort = () => { if (terminal) return; terminal = true; onEvent({ operationId: id, type: "cancelled", message: "Chat stream cancelled." }); finishTerminal?.(); }; signal.addEventListener("abort", onAbort, { once: true }); try { const value = record(await invoke("chat.start", { projectId: idValue(projectId, "projectId"), prompt: boundedString(prompt, "prompt", 20_000), operationId: id }), "chat"); await terminalReached; return { messageId: idValue(value.messageId, "chat.messageId") }; } finally { clearTimeout(timer); signal.removeEventListener("abort", onAbort); unsubscribe(); } },
    ...
    async shareProject(projectId, recipientSlotId, role) { if (!shareRoles.has(role)) throw new BridgeSchemaError("role", "share role"); await invoke("projects.share", { projectId: idValue(projectId, "projectId"), recipientSlotId: idValue(recipientSlotId, "recipientSlotId"), role }); },
