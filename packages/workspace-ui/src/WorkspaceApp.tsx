...
function ErrorNotice({ state }: { state: WorkspaceState }): JSX.Element | null {
...
function AuthPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  ...
  return (
    <section className="auth-card" aria-labelledby="auth-heading">
      ...
      <div className="auth-actions">
        <button type="button" className="primary" onClick={() => void controller.beginBrowserLogin()} disabled={state.auth === "browser-pending" || state.auth === "device-pending"}>
          {state.auth === "browser-pending" ? "Waiting for browser sign-in…" : "Sign in in browser"}
        </button>
        <button type="button" className="secondary" onClick={() => void controller.beginDeviceLogin()} disabled={state.auth === "browser-pending" || state.auth === "device-pending"}>
      ...
      {state.deviceCode && (
        <div className="device-code" role="status" aria-live="polite">
          ...
          <a href={state.deviceCode.verificationUri} target="_blank" rel="noreferrer">Open verification page</a>
        </div>
      )}
            ...
            {accounts.map((account) => (
              <li key={account.id}>
                <div>
                  ...
                  <span className="muted">{account.email} · {account.role}</span>
                </div>
                <button type="button" className="secondary" onClick={() => void controller.selectAccount(account.id)} disabled={!account.ready} title={account.ready ? "Use this account" : "This account is not ready for project access"}>
                  {account.ready ? "Use account" : "Not ready"}
                </button>
              </li>
            ))}
          </ul>
...
function ProjectPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  ...
  return (
    <div className="panel-grid">
      <section className="surface-card" aria-labelledby="projects-heading">
        <div className="section-heading"><div><p className="eyebrow">Workspace</p><h2 id="projects-heading">Projects</h2></div><button type="button" className="secondary" onClick={() => void controller.loadWorkspace()} disabled={state.projectOperation === "loading"}>Refresh</button></div>
...
function ProjectRow({ project, controller, state }: { project: Project; controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  return <li className={state.activeProject?.id === project.id ? "project-row active" : "project-row"}>
    ...
    <button type="button" className="secondary" onClick={() => void controller.openProject(project.id)} disabled={!controller.has("project:open") || state.projectOperation === "loading"} title={controller.has("project:open") ? "Open project" : "A ready authenticated account is required"}>Open</button>
  </li>;
}

function SystemsPanel({ state }: { state: WorkspaceState }): JSX.Element {
...
function EditorPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  ...
  return <div className="editor-layout">
    <section className="surface-card file-tree" aria-labelledby="files-heading"><div className="section-heading"><h2 id="files-heading">Files</h2><span className="meta">{state.files.length} entries</span></div>{state.files.length === 0 ? <p className="empty">Open a project to browse its files.</p> : <ul>{state.files.map((file) => <li key={file.path}><button type="button" className={state.activeFile === file.path ? "file-button active" : "file-button"} onClick={() => file.kind === "file" && void controller.openFile(file.path)} disabled={file.kind !== "file" || !controller.has("file:read")} aria-current={state.activeFile === file.path ? "page" : undefined}><span aria-hidden="true">{file.kind === "folder" ? "▸" : "·"}</span>{file.path}</button></li>)}</ul>}</section>
    <section className="surface-card editor-surface" aria-labelledby="editor-heading"><div className="section-heading"><div><p className="eyebrow">{state.fileLanguage || "Plain text"}</p><h2 id="editor-heading">{state.activeFile || "No file selected"}</h2></div><button type="button" className="primary" onClick={() => void controller.saveFile(draft)} disabled={!state.activeFile || draft === state.fileContent || !controller.has("file:write") || state.fileOperation === "saving"} title={controller.has("file:write") ? "Save file" : "Your active account cannot edit files"}>{state.fileOperation === "saving" ? "Saving…" : "Save"}</button></div><textarea aria-label={state.activeFile ? `Editor for ${state.activeFile}` : "File editor"} value={draft} onChange={(event) => setDraft(event.target.value)} disabled={!state.activeFile || !controller.has("file:write")} spellCheck={false} />{!controller.has("file:write") && state.activeProject && <p className="inline-warning">Read-only role: editing is unavailable for this account.</p>}</section>
  </div>;
}

function PreviewPanel({ state }: { state: WorkspaceState }): JSX.Element {
  return <section className="surface-card" aria-labelledby="preview-heading"><p className="eyebrow">Read-only</p><h2 id="preview-heading">Preview</h2>{state.activeProject ? <><p className="muted">Preview of {state.activeFile || state.activeProject.name}. This surface does not execute project code.</p><pre className="preview" aria-label="Read-only project preview">{state.fileContent || "Select a file in Editor to preview its text."}</pre></> : <p className="empty">Open a project to preview it.</p>}</section>;
}

function ChatPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  const [prompt, setPrompt] = useState("");
  return <section className="surface-card" aria-labelledby="chat-heading"><div className="section-heading"><div><p className="eyebrow">Streamed operation</p><h2 id="chat-heading">Project chat</h2></div><button type="button" className="secondary" onClick={() => controller.cancelChat()} disabled={state.chatOperation !== "streaming"}>Cancel</button></div><div className="chat-log" aria-live="polite">{state.chat.length === 0 ? <p className="empty">No messages yet. Ask about the open project.</p> : state.chat.map((item) => <article key={item.id} className={`chat-message ${item.role}`}><strong>{item.role}</strong><p>{item.text || "Waiting for response…"}</p></article>)}</div><form className="chat-form" onSubmit={(event) => { event.preventDefault(); if (prompt.trim()) { void controller.sendChat(prompt.trim()); setPrompt(""); } }}><label htmlFor="chat-prompt">Message</label><textarea id="chat-prompt" value={prompt} onChange={(event) => setPrompt(event.target.value)} rows={3} disabled={!controller.has("chat") || !state.activeProject || state.chatOperation === "streaming"} placeholder="Ask about this project" /><button type="submit" className="primary" disabled={!prompt.trim() || !controller.has("chat") || !state.activeProject || state.chatOperation === "streaming"}>Send message</button></form></section>;
}

function CommentsPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  const [body, setBody] = useState("");
  const [replyFor, setReplyFor] = useState<string>();
  const [reply, setReply] = useState("");
  return <section className="surface-card" aria-labelledby="comments-heading"><div className="section-heading"><div><p className="eyebrow">Review</p><h2 id="comments-heading">Comments</h2></div></div>{!state.activeProject ? <p className="empty">Open a project to see comments.</p> : <><div className="comment-list">{state.comments.length === 0 ? <p className="empty">No comments yet.</p> : state.comments.map((comment) => <article className="comment" key={comment.id}><strong>{comment.author}</strong><span className="meta">{comment.createdAt}</span><p>{comment.body}</p><button type="button" className="text-button" onClick={() => setReplyFor(replyFor === comment.id ? undefined : comment.id)}>Reply</button>{replyFor === comment.id && <form onSubmit={(event) => { event.preventDefault(); if (reply.trim()) { void controller.replyComment(comment.id, reply.trim()); setReply(""); setReplyFor(undefined); } }}><label htmlFor={`reply-${comment.id}`}>Reply to {comment.author}</label><textarea id={`reply-${comment.id}`} value={reply} onChange={(event) => setReply(event.target.value)} rows={2} /><button type="submit" className="secondary" disabled={!reply.trim() || !controller.has("comment")}>Send reply</button></form>}{comment.replies.map((item) => <div className="reply" key={item.id}><strong>{item.author}</strong><p>{item.body}</p></div>)}</article>)}</div><form onSubmit={(event) => { event.preventDefault(); if (body.trim()) { void controller.addComment(body.trim()); setBody(""); } }}><label htmlFor="new-comment">New comment</label><textarea id="new-comment" value={body} onChange={(event) => setBody(event.target.value)} rows={3} disabled={!controller.has("comment")} /><button type="submit" className="primary" disabled={!body.trim() || !controller.has("comment")} title={controller.has("comment") ? "Add comment" : "Your active account cannot comment"}>Add comment</button></form></>}</section>;
}

function SharePanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  const [recipient, setRecipient] = useState("");
  const [role, setRole] = useState<ShareRole>("can-view");
  return <section className="surface-card narrow-card" aria-labelledby="share-heading"><p className="eyebrow">Access</p><h2 id="share-heading">Share project</h2><p className="muted">Sharing is submitted to the bridge and enforced by the service.</p><form onSubmit={(event) => { event.preventDefault(); if (recipient.trim()) { void controller.share(recipient.trim(), role); setRecipient(""); } }}><label htmlFor="share-recipient">Account or email</label><input id="share-recipient" value={recipient} onChange={(event) => setRecipient(event.target.value)} required disabled={!state.activeProject || !controller.has("share")} /><label htmlFor="share-role">Role</label><select id="share-role" value={role} onChange={(event) => setRole(event.target.value as ShareRole)} disabled={!state.activeProject || !controller.has("share")}><option value="can-view">Can view</option><option value="can-comment">Can comment</option><option value="can-edit">Can edit</option></select><button type="submit" className="primary" disabled={!recipient.trim() || !state.activeProject || !controller.has("share")}>Share</button></form>{!controller.has("share") && state.activeProject && <p className="inline-warning">Your role cannot change sharing.</p>}</section>;
}

function SettingsPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  const [compact, setCompact] = useState(Boolean(state.settings.compact));
  return <section className="surface-card narrow-card" aria-labelledby="settings-heading"><p className="eyebrow">Workspace preferences</p><h2 id="settings-heading">Settings</h2><label className="switch-row"><input type="checkbox" checked={compact} onChange={(event) => setCompact(event.target.checked)} disabled={!controller.has("settings")} /><span>Compact project rows</span></label><p className="muted">Stored through the typed bridge. The active account must have settings access.</p><button type="button" className="primary" onClick={() => void controller.saveSettings({ compact })} disabled={!controller.has("settings")}>Save settings</button></section>;
}

function UtilityPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  return <section className="surface-card" aria-labelledby="utilities-heading"><p className="eyebrow">Recovery and tools</p><h2 id="utilities-heading">Utilities</h2><div className="utility-grid"><button type="button" className="secondary" onClick={() => void controller.loadWorkspace()} disabled={!state.activeAccount || state.projectOperation === "loading"}>Reload workspace</button><button type="button" className="secondary" onClick={() => controller.signOut()}>Sign out</button><span className="unavailable" role="note">Local converter and authenticator are unavailable in this bridge version.</span></div></section>;
}

function DocsPanel(): JSX.Element { return <section className="surface-card" aria-labelledby="docs-heading"><p className="eyebrow">Offline reference</p><h2 id="docs-heading">Documentation</h2><div className="docs-list">{docs.map(([title, body]) => <details key={title}><summary>{title}</summary><p>{body}</p></details>)}</div></section>; }
function SimplePanel({ id, title, body, state }: { id: string; title: string; body: string; state: WorkspaceState }): JSX.Element { return <section className="surface-card" aria-labelledby={`${id}-heading`}><p className="eyebrow">Workspace state</p><h2 id={`${id}-heading`}>{title}</h2><p className="muted">{body}</p><dl className="status-list"><div><dt>Authentication</dt><dd>{state.auth}</dd></div><div><dt>Active account</dt><dd>{state.activeAccount?.label ?? "None"}</dd></div><div><dt>Open project</dt><dd>{state.activeProject?.name ?? "None"}</dd></div></dl></section>; }

function TabPanel({ id, controller, state }: { id: string; controller: WorkspaceController; state: WorkspaceState }): JSX.Element {
  switch (id) {
    ...
    case "preview": return <PreviewPanel state={state} />;
...
export function WorkspaceApp({ bridge }: { bridge: DesignerBridge }): JSX.Element {
  ...
  const moveTab = (delta: number) => selectTab(tabs[(activeIndex + delta + tabs.length) % tabs.length].id, true);
  ...
  return <main className="workspace-shell"><header className="workspace-header"><div><p className="eyebrow">Design workspace</p><h1>{state.activeProject?.name ?? "Choose a project"}</h1><p className="muted">{state.activeAccount?.label} · {state.activeAccount?.role}</p></div><button type="button" className="secondary" onClick={() => controller.signOut()}>Sign out</button></header><ErrorNotice state={state} /><nav className="tab-strip" aria-label="Workspace sections"><div role="tablist" aria-orientation="horizontal">{tabs.map((tab) => <button key={tab.id} ref={(element) => { tabRefs.current[tab.id] = element; }} id={`tab-${tab.id}`} type="button" role="tab" aria-selected={activeTab === tab.id} aria-controls={`panel-${tab.id}`} tabIndex={activeTab === tab.id ? 0 : -1} className={activeTab === tab.id ? "tab active" : "tab"} onClick={() => selectTab(tab.id)} onKeyDown={(event) => { if (event.key === "ArrowRight") { event.preventDefault(); moveTab(1); } else if (event.key === "ArrowLeft") { event.preventDefault(); moveTab(-1); } else if (event.key === "Home") { event.preventDefault(); selectTab(tabs[0].id, true); } else if (event.key === "End") { event.preventDefault(); selectTab(tabs[tabs.length - 1].id, true); } }}>{tab.label}<span className="sr-only">: {tab.description}</span></button>)}</div></nav><section id={`panel-${activeTab}`} role="tabpanel" aria-labelledby={`tab-${activeTab}`} tabIndex={0} className="tab-panel"><TabPanel id={activeTab} controller={controller} state={state} /></section></main>;
}
