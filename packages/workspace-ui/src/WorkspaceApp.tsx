          ...
          <ul className="account-list" aria-label="Signed-in accounts">
            {accounts.map((account) => (
              <li key={account.slotId}>
                ...
                <button type="button" className="secondary" onClick={() => void controller.selectAccount(account.slotId)} disabled={account.state !== "ready"} title={account.state === "ready" ? "Use this account" : "This account is not ready for project access"}>
...
function PreviewPanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): ReactElement {
  return <section className="surface-card" aria-labelledby="preview-heading"><div className="section-heading"><div><p className="eyebrow">Host-rendered</p><h2 id="preview-heading">Preview</h2></div><div className="inline-actions"><button type="button" className="primary" onClick={() => void controller.openPreview()} disabled={!state.activeProject || !controller.has("file:read") || state.previewOperation === "loading"}>Open preview</button><button type="button" className="secondary" onClick={() => void controller.closePreview()} disabled={!state.preview}>Close preview</button></div></div>{state.preview ? <><p className="muted">{state.preview.title}{state.preview.expiresAt ? ` · expires ${state.preview.expiresAt}` : ""}</p><iframe className="preview-frame" title={state.preview.title} src={state.preview.url} sandbox="allow-scripts" /></> : <p className="empty">Open a project, then request a preview handle from the host.</p>}</section>;
}
...
function SharePanel({ controller, state }: { controller: WorkspaceController; state: WorkspaceState }): ReactElement {
  ...
  return <section className="surface-card narrow-card" aria-labelledby="share-heading"><p className="eyebrow">Access</p><h2 id="share-heading">Share project</h2><p className="muted">Sharing is submitted to the bridge and enforced by the service. Only the project owner can grant, revoke, or transfer access.</p><form onSubmit={(event) => { event.preventDefault(); if (recipient) void controller.share(recipient, role); }}><label htmlFor="share-recipient">Saved account slot</label><select id="share-recipient" value={recipient} onChange={(event) => setRecipient(event.target.value)} required disabled={!state.activeProject || !controller.has("share")}><option value="">Choose an account</option>{savedSlots.map((account) => <option key={account.slotId} value={account.slotId}>{account.label} · {account.email}</option>)}</select><label htmlFor="share-role">Role</label><select id="share-role" value={role} onChange={(event) => setRole(event.target.value as ShareRole)} disabled={!state.activeProject || !controller.has("share")}><option value="can-view">Can view</option><option value="can-comment">Can comment</option><option value="can-edit">Can edit</option></select><div className="inline-actions"><button type="submit" className="primary" disabled={!recipient || !state.activeProject || !controller.has("share")}>Grant access</button><button type="button" className="secondary" onClick={() => recipient && void controller.revokeShare(recipient)} disabled={!recipient || !state.activeProject || !controller.has("share")}>Revoke access</button><button type="button" className="secondary" onClick={() => recipient && void controller.transferProject(recipient)} disabled={!recipient || !state.activeProject || !controller.has("transfer")}>Transfer ownership</button></div></form>{!controller.has("share") && state.activeProject && <p className="inline-warning">Only a project owner can change sharing.</p>}</section>;
}
