...
export type McpBridgeOptions = { accountId: string; previewOrigin: string; planTokens?: PlanTokenManager; maxLineBytes?: number };
...
export class McpBridge {
  ...
  private async call(name: McpToolName, a: Record<string, any>): Promise<unknown> {
    ...
    switch (name) {
      ...
      case "finalize_plan": return text({ ...this.plans.issue(id(), a.scope === "project" ? "project" : "paths", a.writes || [], a.deletes || []), project_id: id() });
      case "write_files": { const files = (Array.isArray(a.files) ? a.files : []).map((f: any) => ({ path: normalizeProjectPath(f.path), content: bytes(f), contentType: f.content_type || f.contentType, ifMatch: f.if_match })); const r = await this.domain.writeFiles(this.ctx, id(), files); return text({ data: r.files.map(fil), files: r.files.map(fil), project_id: id() }); }
      case "copy_files": { const fs = (a.files || []).map((f: any) => ({ src: normalizeProjectPath(f.src), dest: normalizeProjectPath(f.dest), srcProjectId: f.src_project_id, ifMatch: f.if_match })); this.plans.validate(a.plan_token, id(), fs.map((f: any) => f.dest), "writes"); return text(await this.domain.copyFiles(this.ctx, id(), fs)); }
      case "delete_files": { const ps = [...(a.paths || []), ...(a.files || []).map((f: any) => f.path)].map(normalizeProjectPath); this.plans.validate(a.plan_token, id(), ps, "deletes"); return text(await this.domain.deleteFiles(this.ctx, id(), ps)); }
  ...
  listenPipe(path: string, onError?: (error: Error) => void) { if (!path) throw new DomainError("A pipe path is required.", 400, "invalid_pipe"); const server = createNetServer((socket) => { let buffer = ""; socket.setEncoding("utf8"); socket.on("data", async (chunk) => { buffer += chunk; let i = buffer.indexOf("\n"); while (i >= 0) { const line = buffer.slice(0, i); buffer = buffer.slice(i + 1); i = buffer.indexOf("\n"); try { socket.write(`${JSON.stringify(await this.handle(JSON.parse(line)))}\n`); } catch { socket.write(`${JSON.stringify(err(null, -32700, "Invalid JSON."))}\n`); } } }); }); if (onError) server.on("error", onError); server.listen(path); return server; }
}
